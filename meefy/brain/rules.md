...
2. **MVP Scope Lock (CRITICAL & ENFORCED)**
3. AI Usage Rules
4. Code Style
5. TypeScript Rules
6. Error Handling
...
## 2. 🔒 MVP SCOPE LOCK (STRICTLY ENFORCED)

**These are the ONLY features that exist until the MVP is deployed and verified with real users:**

### In MVP Scope (Build ONLY These Now):
* [ ] **Visitor OTP-verified request form** (`/request`)
* [ ] **Receptionist queue and scheduling screen** (`/receptionist/queue`, `/receptionist/schedule`)
* [ ] **Officer approval / decline** (`/officer/pending`)
* [ ] **Signed QR pass issued by email + SMS** (`/status/:token`)
* [ ] **Gate QR scan and check-in** (`/receptionist/scan`)
* [ ] **Check-out** (manual scan + auto-close cron)
* [ ] **Basic notifications** (email + SMS only — no push in MVP)
* [ ] **Live occupancy board** (`/receptionist/occupancy`)
* [ ] **Admin user management** (`/admin/users`)
* [ ] **Blacklist enforcement** (`/admin/blacklist`)

---

### ⛔ Explicitly NOT in MVP Scope — [AFTER MVP]:
**The following topics are BANNED from active development until the MVP ships:**
* **[AFTER MVP]** Push notifications (Web Push)
* **[AFTER MVP]** Officer-initiated invites
* **[AFTER MVP]** Walk-in express lane
* **[AFTER MVP]** Group visits
* **[AFTER MVP]** Recurring visitor passes
* **[AFTER MVP]** Reports and analytics dashboard
* **[AFTER MVP]** Mobile native apps (iOS / Android)
* **[AFTER MVP]** Room / conference resource booking
* **[AFTER MVP]** Badge printing hardware integrations
* **[AFTER MVP]** LDAP / HR system integration
* **[AFTER MVP]** Dark mode
* **[AFTER MVP]** Internationalisation (i18n)
* **[AFTER MVP]** Any AI or ML feature
...
## 6. Error Handling
This is the most critical section. Every execution path that can fail must handle the failure explicitly. No exceptions. No lazy re-throws. No empty catch blocks.
...
* No secret in code. No API keys, JWT secrets, or DB credentials anywhere in the codebase. Environment variables only.
...
* CORS locked to known origins. Never `origin: '*'` in production.
...
## 11. What to Use
These packages are approved. Use these and only these unless a new package is discussed and approved.
...
* `socket.io` — **[AFTER MVP]** Not needed in MVP — polling is fine for occupancy board
...
## 14. Git Rules
* Branch naming: `feature/visitor-request-form`, `fix/otp-lockout-not-clearing`, `chore/add-drizzle-indexes`, `docs/update-architecture`
...
> **Write code for that 9am Monday morning situation at the gate. Everything else follows.**